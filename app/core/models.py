...
class CategoryLink(Base):
    ...
    link_id = Column(
        Integer,
        ForeignKey(column='links.id', ondelete='CASCADE'),
        primary_key=True
    )
    category = relationship(
        argument="Category",
        back_populates="link_associations"
    )
    link = relationship(
        argument="Link",
...
class Category(Base):
    ...
    links = association_proxy(
        target_collection="link_associations",
        attr="link"
    )
...
class Link(Base):
    ...
    categories = association_proxy(
        target_collection="category_associations",
        attr="category"
    )
...
class ChannelSimilar(Base):
    ...
    main_channel_id = Column(
        Integer,
        ForeignKey(column='channels.id', ondelete='CASCADE'),
        primary_key=True
    )
    ...
    main_channel = relationship(
        "Channel",
        foreign_keys=[main_channel_id],
        back_populates="similar_to"
    )
    similar_channel = relationship(
        "Channel",
        foreign_keys=[similar_channel_id],
        back_populates="similar_by"
    )
...
class ChannelRelated(Base):
    ...
    related_channel = relationship(
        "Channel",
        foreign_keys=[related_channel_id],
